...
(define (ball-after-tick bl r paused?)
  (cond
    [paused? bl]
    [(empty? bl) empty]
    [(ball-collides-with-bottom-wall? (first bl))
     (ball-after-tick (rest bl) r paused?)]
    [else (cons (ball-collides (first bl) r)
                (ball-after-tick (rest bl) r paused?))]))
...
;;           => INITIAL-RACKET
;; DESIGN STRATEGY : conditions on the x co-ordinate of the racket.
...
(define (racket-collision-with-ball r bl)
  (cond
    [(empty? bl) r]
    [(ball-collides-with-racket? r (first bl))
     (racket-after-collision-with-ball r)]
    [else (racket-collision-with-ball r (rest bl))]))
...
;; DESIGN STRATEGY : use the observer template for BallList.

(define (ball-list-images bl)
  (cond
    [(empty? bl) empty]
    [else (cons BALL (ball-list-images (rest bl)))]))
...
(define (ball-list-posns bl)
  (cond
    [(empty? bl) empty]
    [else (cons (make-posn (ball-x (first bl))
                           (ball-y (first bl)))
                (ball-list-posns (rest bl)))]))
